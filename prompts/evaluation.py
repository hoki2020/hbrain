...
判断 summary 是否符合该类型的总结要求（见下方各类型要求）。
注意：Rule 和 Image 类型的 summary 必须是 JSON 结构，不是自然语言。如果 Rule 的 summary 是合法 JSON 且包含必要字段（rule_type、scope、constraint、judgement 等），则描述质量应评为高分。不要因为 Rule 的 summary 是 JSON 格式而扣分——这是该类型的要求。
...
Rule：summary 必须是 JSON 结构。检查是否包含 rule_type、scope（适用范围）、condition（触发条件）、constraint（约束）、exceptions（例外）、judgement（判断逻辑）。如果 JSON 结构完整且字段合理，应给高分（0.7-1.0）。不要因为是 JSON 格式而非自然语言就扣分。
...
Image：summary 必须是 JSON 结构。检查是否包含 doc_id、image_url、caption。caption 是否详细描述了图片内容。
